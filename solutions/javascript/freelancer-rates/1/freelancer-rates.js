...
export function dayRate(ratePerHour) {
    ...
    return ratePerHour * hoursWorkedInAday;
}
...
export function daysInBudget(budget, ratePerHour) {
    const hoursWorkedInAday = 8;
    return Math.floor(budget / (ratePerHour * hoursWorkedInAday));
}
...
export function priceWithMonthlyDiscount(hourlyRate, numDays, discount) {
    ...
    const monthlyRate = hourlyRate * hoursPerDay * daysPerMonth;
    const discountedMonthlyRate = monthlyRate * (1 - discount);

    const remainingRate = remainingDays * hoursPerDay * hourlyRate;
